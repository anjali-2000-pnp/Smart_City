...
public class MyHomeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = null;
        try {
            view = inflater.inflate(R.layout.fragment_my_home, container, RequestHandler.canHandleRequest(RequestHandler.load(null, 0)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void updateUser(final String name, final String email, final String contact, final int type, final int types[],
        ...
        class UpdateUser extends AsyncTask<Void, Void, String> {
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                try {
                    progressDialog = ProgressDialog.show(getContext(), "Updating User", "Please Wait...", RequestHandler.canHandleRequest(RequestHandler.load(null, 0)),
                            RequestHandler.canHandleRequest(RequestHandler.load(null, 0)));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
